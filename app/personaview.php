...
class cpersona_view extends cpersona {
	...
	function Page_Init() {
		...
		if (@$_GET["idpersona"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["idpersona"]);
		}
	...
	function SetupOtherOptions() {
		...
		$item = &$option->Add("detail_persona_fotografia");
		$body = $Language->Phrase("ViewPageDetailLink") . $Language->TablePhrase("persona_fotografia", "TblCaption");
		...
		$links = "";
		if ($GLOBALS["persona_fotografia_grid"] && $GLOBALS["persona_fotografia_grid"]->DetailView && $Security->IsLoggedIn() && $Security->IsLoggedIn()) {
		...
		if ($GLOBALS["persona_fotografia_grid"] && $GLOBALS["persona_fotografia_grid"]->DetailEdit && $Security->IsLoggedIn() && $Security->IsLoggedIn()) {
	...
	function ExportData() {
		...
		// Export detail records (persona_fotografia)
		if (EW_EXPORT_DETAIL_RECORDS && in_array("persona_fotografia", explode(",", $this->getCurrentDetailTable()))) {
			global $persona_fotografia;
			if (!isset($persona_fotografia)) $persona_fotografia = new cpersona_fotografia;
			$rsdetail = $persona_fotografia->LoadRs($persona_fotografia->GetDetailFilter()); // Load detail records
			if ($rsdetail && !$rsdetail->EOF) {
				$ExportStyle = $Doc->Style;
				$Doc->SetStyle("h"); // Change to horizontal
				if ($this->Export <> "csv" || EW_EXPORT_DETAIL_RECORDS_FOR_CSV) {
					$Doc->ExportEmptyRow();
					$detailcnt = $rsdetail->RecordCount();
					$persona_fotografia->ExportDocument($Doc, $rsdetail, 1, $detailcnt);
				}
				$Doc->SetStyle($ExportStyle); // Restore
				$rsdetail->Close();
			}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("persona_fotografia", $DetailTblVar)) {
				...
				if ($GLOBALS["persona_fotografia_grid"]->DetailView) {
					$GLOBALS["persona_fotografia_grid"]->CurrentMode = "view";
...
<?php if ($persona->telefonos->Visible) { // telefonos ?>
	<tr id="r_telefonos">
		<td><span id="elh_persona_telefonos"><?php echo $persona->telefonos->FldCaption() ?></span></td>
...
<?php echo $persona->telefonos->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if ($persona->fotografia->Visible) { // fotografia ?>
	<tr id="r_fotografia">
		<td><span id="elh_persona_fotografia"><?php echo $persona->fotografia->FldCaption() ?></span></td>
		<td data-name="fotografia"<?php echo $persona->fotografia->CellAttributes() ?>>
<span id="el_persona_fotografia">
<span>
<?php echo ew_GetFileViewTag($persona->fotografia, $persona->fotografia->ViewValue) ?>
</span>
</span>
</td>
	</tr>
...
<?php
	if (in_array("puesto", explode(",", $persona->getCurrentDetailTable())) && $puesto->DetailView) {
?>
...
<?php } ?>
<?php
	if (in_array("persona_fotografia", explode(",", $persona->getCurrentDetailTable())) && $persona_fotografia->DetailView) {
?>
<?php if ($persona->getCurrentDetailTable() <> "") { ?>
