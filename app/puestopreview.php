...
$puesto_preview = NULL; // Initialize page object first

class cpuesto_preview extends cpuesto {
	...
	// Page object name
	var $PageObjName = 'puesto_preview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'puesto');
		if (!$Security->CanList()) {
			echo ew_DeniedMsg();
			exit();
		}
	...
	function MasterKeyUrl() {
		...
		if ($mastertblvar == "puesto_tipo") {
			$url = "" . EW_TABLE_SHOW_MASTER . "=puesto_tipo&fk_idpuesto_tipo=" . urlencode(strval($this->idpuesto_tipo->QueryStringValue)) . "";
		}
		return $url;
	}
	...
	function SetupForeignKeysFromFilter($f) {
		...
		if ($mastertblvar == "puesto_tipo") {
			...
			if ($x !== FALSE) {
				...
				$val = substr($f, $x);
				$val = $this->UnquoteValue($val, "DB");
 				$this->idpuesto_tipo->setQueryStringValue($val);
			}
		}
	}
...
// Create page object
if (!isset($puesto_preview)) $puesto_preview = new cpuesto_preview();

// Page init
$puesto_preview->Page_Init();

// Page main
$puesto_preview->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$puesto_preview->Page_Render();
?>
<?php $puesto_preview->ShowPageHeader(); ?>
<?php if ($puesto_preview->TotalRecs > 0) { ?>
<div class="ewGrid">
...
// Render list options
$puesto_preview->RenderListOptions();

// Render list options (header, left)
$puesto_preview->ListOptions->Render("header", "left");
?>
<?php if ($puesto->idpuesto_tipo->Visible) { // idpuesto_tipo ?>
			<th><?php echo $puesto->idpuesto_tipo->FldCaption() ?></th>
<?php } ?>
<?php if ($puesto->idpersona->Visible) { // idpersona ?>
			<th><?php echo $puesto->idpersona->FldCaption() ?></th>
<?php } ?>
<?php if ($puesto->idfederacion->Visible) { // idfederacion ?>
			<th><?php echo $puesto->idfederacion->FldCaption() ?></th>
<?php } ?>
<?php if ($puesto->idorgano->Visible) { // idorgano ?>
			<th><?php echo $puesto->idorgano->FldCaption() ?></th>
<?php } ?>
<?php if ($puesto->fecha_inicio->Visible) { // fecha_inicio ?>
			<th><?php echo $puesto->fecha_inicio->FldCaption() ?></th>
<?php } ?>
<?php if ($puesto->fecha_fin->Visible) { // fecha_fin ?>
			<th><?php echo $puesto->fecha_fin->FldCaption() ?></th>
<?php } ?>
<?php if ($puesto->status->Visible) { // status ?>
			<th><?php echo $puesto->status->FldCaption() ?></th>
<?php } ?>
<?php

// Render list options (header, right)
$puesto_preview->ListOptions->Render("header", "right");
?>
		</tr>
	</thead>
	<tbody><!-- Table body -->
<?php
$puesto_preview->RecCount = 0;
$puesto_preview->RowCnt = 0;
while ($puesto_preview->Recordset && !$puesto_preview->Recordset->EOF) {

	// Init row class and style
	$puesto_preview->RecCount++;
	$puesto_preview->RowCnt++;
	$puesto_preview->CssStyle = "";
	$puesto_preview->LoadListRowValues($puesto_preview->Recordset);

	// Render row
	$puesto_preview->RowType = EW_ROWTYPE_PREVIEW; // Preview record
	$puesto_preview->RenderListRow();

	// Render list options
	$puesto_preview->RenderListOptions();
?>
	<tr<?php echo $puesto_preview->RowAttributes() ?>>
<?php

// Render list options (body, left)
$puesto_preview->ListOptions->Render("body", "left", $puesto_preview->RowCnt);
?>
<?php if ($puesto->idpuesto_tipo->Visible) { // idpuesto_tipo ?>
		<!-- idpuesto_tipo -->
		<td<?php echo $puesto->idpuesto_tipo->CellAttributes() ?>>
...
<?php if ($puesto->idpersona->Visible) { // idpersona ?>
		<!-- idpersona -->
		<td<?php echo $puesto->idpersona->CellAttributes() ?>>
...
<?php if ($puesto->idfederacion->Visible) { // idfederacion ?>
		<!-- idfederacion -->
		<td<?php echo $puesto->idfederacion->CellAttributes() ?>>
...
<?php if ($puesto->idorgano->Visible) { // idorgano ?>
		<!-- idorgano -->
		<td<?php echo $puesto->idorgano->CellAttributes() ?>>
...
<?php if ($puesto->fecha_inicio->Visible) { // fecha_inicio ?>
		<!-- fecha_inicio -->
		<td<?php echo $puesto->fecha_inicio->CellAttributes() ?>>
...
<?php if ($puesto->fecha_fin->Visible) { // fecha_fin ?>
		<!-- fecha_fin -->
		<td<?php echo $puesto->fecha_fin->CellAttributes() ?>>
...
<?php if ($puesto->status->Visible) { // status ?>
		<!-- status -->
		<td<?php echo $puesto->status->CellAttributes() ?>>
...
$puesto_preview->ListOptions->Render("body", "right", $puesto_preview->RowCnt);
?>
	</tr>
<?php
	$puesto_preview->Recordset->MoveNext();
}
?>
	</tbody>
</table>
...
<?php if ($puesto_preview->TotalRecs > 0) { ?>
<div class="ewDetailCount"><?php echo $puesto_preview->TotalRecs ?>&nbsp;<?php echo $Language->Phrase("Record") ?></div>
<?php } else { ?>
<div class="ewDetailCount"><?php echo $Language->Phrase("NoRecord") ?></div>
<?php } ?>
<div class="ewPreviewOtherOptions">
<?php
	foreach ($puesto_preview->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
...
<?php
$puesto_preview->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php
if ($puesto_preview->Recordset)
	$puesto_preview->Recordset->Close();
