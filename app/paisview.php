...
class cpais_view extends cpais {
	...
	function SetupOtherOptions() {
		...
		$body = $Language->Phrase("ViewPageDetailLink") . $Language->TablePhrase("departamento", "TblCaption");
		...
		$links = "";
		if ($GLOBALS["departamento_grid"] && $GLOBALS["departamento_grid"]->DetailView && $Security->IsLoggedIn() && $Security->IsLoggedIn()) {
		...
		if ($GLOBALS["departamento_grid"] && $GLOBALS["departamento_grid"]->DetailEdit && $Security->IsLoggedIn() && $Security->IsLoggedIn()) {
	...
	function ExportData() {
		...
		// Export detail records (departamento)
		if (EW_EXPORT_DETAIL_RECORDS && in_array("departamento", explode(",", $this->getCurrentDetailTable()))) {
			global $departamento;
			if (!isset($departamento)) $departamento = new cdepartamento;
			$rsdetail = $departamento->LoadRs($departamento->GetDetailFilter()); // Load detail records
			if ($rsdetail && !$rsdetail->EOF) {
				...
				if ($this->Export <> "csv" || EW_EXPORT_DETAIL_RECORDS_FOR_CSV) {
					...
					$departamento->ExportDocument($Doc, $rsdetail, 1, $detailcnt);
				}
...
<?php if ($pais->gentilicio_masculino->Visible) { // gentilicio_masculino ?>
	<tr id="r_gentilicio_masculino">
		<td><span id="elh_pais_gentilicio_masculino"><?php echo $pais->gentilicio_masculino->FldCaption() ?></span></td>
...
<?php echo $pais->gentilicio_masculino->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pais->gentilicio_femenino->Visible) { // gentilicio_femenino ?>
	<tr id="r_gentilicio_femenino">
		<td><span id="elh_pais_gentilicio_femenino"><?php echo $pais->gentilicio_femenino->FldCaption() ?></span></td>
...
<?php echo $pais->gentilicio_femenino->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
