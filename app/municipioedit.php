...
class cmunicipio_edit extends cmunicipio {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// iddepartamento
			$this->iddepartamento->SetDbValueDef($rsnew, $this->iddepartamento->CurrentValue, NULL, $this->iddepartamento->ReadOnly);
...
fmunicipioedit.Lists["x_iddepartamento"] = {"LinkField":"x_iddepartamento","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fmunicipioedit.Lists["x_estado"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fmunicipioedit.Lists["x_estado"].Options = <?php echo json_encode($municipio->estado->Options()) ?>;
...
<?php if ($municipio->getCurrentMasterTable() == "departamento") { ?>
...
<?php if ($municipio->siglas->Visible) { // siglas ?>
	<div id="r_siglas" class="form-group">
		<label id="elh_municipio_siglas" for="x_siglas" class="col-sm-2 control-label ewLabel"><?php echo $municipio->siglas->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $municipio->siglas->CellAttributes() ?>>
<span id="el_municipio_siglas">
<input type="text" data-table="municipio" data-field="x_siglas" name="x_siglas" id="x_siglas" size="30" maxlength="45" placeholder="<?php echo ew_HtmlEncode($municipio->siglas->getPlaceHolder()) ?>" value="<?php echo $municipio->siglas->EditValue ?>"<?php echo $municipio->siglas->EditAttributes() ?>>
</span>
<?php echo $municipio->siglas->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($municipio->iddepartamento->Visible) { // iddepartamento ?>
	<div id="r_iddepartamento" class="form-group">
		<label id="elh_municipio_iddepartamento" for="x_iddepartamento" class="col-sm-2 control-label ewLabel"><?php echo $municipio->iddepartamento->FldCaption() ?></label>
...
<?php } else { ?>
<span id="el_municipio_iddepartamento">
<select data-table="municipio" data-field="x_iddepartamento" data-value-separator="<?php echo ew_HtmlEncode(is_array($municipio->iddepartamento->DisplayValueSeparator) ? json_encode($municipio->iddepartamento->DisplayValueSeparator) : $municipio->iddepartamento->DisplayValueSeparator) ?>" id="x_iddepartamento" name="x_iddepartamento"<?php echo $municipio->iddepartamento->EditAttributes() ?>>
<?php
...
?>
<input type="hidden" name="s_x_iddepartamento" id="s_x_iddepartamento" value="<?php echo $municipio->iddepartamento->LookupFilterQuery() ?>">
</span>
<?php } ?>
...
<select data-table="municipio" data-field="x_estado" data-value-separator="<?php echo ew_HtmlEncode(is_array($municipio->estado->DisplayValueSeparator) ? json_encode($municipio->estado->DisplayValueSeparator) : $municipio->estado->DisplayValueSeparator) ?>" id="x_estado" name="x_estado"<?php echo $municipio->estado->EditAttributes() ?>>
<?php
if (is_array($municipio->estado->EditValue)) {
	...
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = ew_SameStr($municipio->estado->CurrentValue, $arwrk[$rowcntwrk][0]) ? " selected" : "";
		if ($selwrk <> "") $emptywrk = FALSE;		
?>
<option value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?>>
<?php echo $municipio->estado->DisplayValue($arwrk[$rowcntwrk]) ?>
</option>
<?php
	}
	if ($emptywrk && strval($municipio->estado->CurrentValue) <> "") {
?>
<option value="<?php echo ew_HtmlEncode($municipio->estado->CurrentValue) ?>" selected><?php echo $municipio->estado->CurrentValue ?></option>
<?php
    }
}
?>
...
<input type="hidden" data-table="municipio" data-field="x_idmunicipio" name="x_idmunicipio" id="x_idmunicipio" value="<?php echo ew_HtmlEncode($municipio->idmunicipio->CurrentValue) ?>">
