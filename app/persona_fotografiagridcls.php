...
$persona_fotografia_grid = NULL; // Initialize page object first

class cpersona_fotografia_grid extends cpersona_fotografia {
	...
	// Page object name
	var $PageObjName = 'persona_fotografia_grid';

	// Grid form hidden field names
	var $FormName = 'fpersona_fotografiagrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["persona_fotografia"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$this->idpersona_fotografia->setFormValue($arrKeyFlds[0]);
			if (!is_numeric($this->idpersona_fotografia->FormValue))
				return FALSE;
		}
		return TRUE;
	}
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->idpersona_fotografia->CurrentValue;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->idpersona_fotografia->CurrentValue . "\">";
		}
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('idpersona_fotografia');
	}
	...
	function LoadDefaultValues() {
		...
		$this->fotografia->Upload->Index = $this->RowIndex;
	...
	function LoadFormValues() {
		...
		if (!$this->idpersona_fotografia->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
			$this->idpersona_fotografia->setFormValue($objForm->GetValue("x_idpersona_fotografia"));
	}
	...
	function RestoreFormValues() {
		...
		if ($this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
			$this->idpersona_fotografia->CurrentValue = $this->idpersona_fotografia->FormValue;
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idpersona_fotografia->CurrentValue = strval($arKeys[0]); // idpersona_fotografia
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		$this->estado->ViewCustomAttributes = "";
			...
			if ($this->fotografia->UseColorbox) {
				...
				$this->fotografia->LinkAttrs["data-rel"] = "persona_fotografia_x" . $this->RowCnt . "_fotografia";
			...
			if (is_numeric($this->RowIndex) && !$this->EventCancelled) ew_RenderUploadField($this->fotografia, $this->RowIndex);

			// fecha_insercion
			$this->fecha_insercion->EditAttrs["class"] = "form-control";
			$this->fecha_insercion->EditCustomAttributes = "";
			$this->fecha_insercion->EditValue = ew_HtmlEncode($this->fecha_insercion->CurrentValue);
	...
	function DeleteRows() {
		...
		if ($DeleteRows) {
			...
			foreach ($rsold as $row) {
				...
				$sThisKey .= $row['idpersona_fotografia'];
				$conn->raiseErrorFn = $GLOBALS["EW_ERROR_FN"];
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// fecha_insercion
			$this->fecha_insercion->SetDbValueDef($rsnew, $this->fecha_insercion->CurrentValue, NULL, $this->fecha_insercion->ReadOnly);
	...
	function AddRow($rsold = NULL) {
		...
		// fecha_insercion
		$this->fecha_insercion->SetDbValueDef($rsnew, $this->fecha_insercion->CurrentValue, NULL, FALSE);
