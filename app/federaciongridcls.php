...
$federacion_grid = NULL; // Initialize page object first

class cfederacion_grid extends cfederacion {
	...
	// Page object name
	var $PageObjName = 'federacion_grid';

	// Grid form hidden field names
	var $FormName = 'ffederaciongrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["federacion"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->idfederacion->CurrentValue;
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_nomenclatura") && $objForm->HasValue("o_nomenclatura") && $this->nomenclatura->CurrentValue <> $this->nomenclatura->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_idfederacion_tipo") && $objForm->HasValue("o_idfederacion_tipo") && $this->idfederacion_tipo->CurrentValue <> $this->idfederacion_tipo->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_iddeporte") && $objForm->HasValue("o_iddeporte") && $this->iddeporte->CurrentValue <> $this->iddeporte->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->idfederacion->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		...
		$this->idfederacion_tipo->OldValue = $this->idfederacion_tipo->CurrentValue;
		$this->iddeporte->CurrentValue = 1;
		$this->iddeporte->OldValue = $this->iddeporte->CurrentValue;
	}
	...
	function LoadFormValues() {
		...
		$this->idfederacion_tipo->setOldValue($objForm->GetValue("o_idfederacion_tipo"));
		if (!$this->iddeporte->FldIsDetailKey) {
			$this->iddeporte->setFormValue($objForm->GetValue("x_iddeporte"));
		}
		$this->iddeporte->setOldValue($objForm->GetValue("o_iddeporte"));
		if (!$this->idfederacion->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idfederacion->CurrentValue = strval($arKeys[0]); // idfederacion
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "federacion_tipo") {
			$this->idfederacion_tipo->Visible = FALSE;
			if ($GLOBALS["federacion_tipo"]->EventCancelled) $this->EventCancelled = TRUE;
		}
		if ($sMasterTblVar == "deporte") {
			$this->iddeporte->Visible = FALSE;
			if ($GLOBALS["deporte"]->EventCancelled) $this->EventCancelled = TRUE;
		}
