...
$puesto_edit = NULL; // Initialize page object first

class cpuesto_edit extends cpuesto {
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->idpuesto->CurrentValue == "")
			$this->Page_Terminate("puestolist.php"); // Invalid key, return to list
	...
	function LoadFormValues() {
		...
		if (!$this->idpuesto->FldIsDetailKey)
...
// Create page object
if (!isset($puesto_edit)) $puesto_edit = new cpuesto_edit();

// Page init
$puesto_edit->Page_Init();

// Page main
$puesto_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$puesto_edit->Page_Render();
?>
...
var CurrentForm = fpuestoedit = new ew_Form("fpuestoedit", "edit");

// Validate form
fpuestoedit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $puesto->estado->FldCaption(), $puesto->estado->ReqErrMsg)) ?>");
...
fpuestoedit.Form_CustomValidate = 
...
fpuestoedit.ValidateRequired = true;
<?php } else { ?>
fpuestoedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fpuestoedit.Lists["x_idpuesto_tipo"] = {"LinkField":"x_idpuesto_tipo","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fpuestoedit.Lists["x_idpersona"] = {"LinkField":"x_idpersona","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fpuestoedit.Lists["x_idfederacion"] = {"LinkField":"x_idfederacion","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fpuestoedit.Lists["x_idorgano"] = {"LinkField":"x_idorgano","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fpuestoedit.Lists["x_status"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fpuestoedit.Lists["x_status"].Options = <?php echo json_encode($puesto->status->Options()) ?>;
fpuestoedit.Lists["x_estado"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fpuestoedit.Lists["x_estado"].Options = <?php echo json_encode($puesto->estado->Options()) ?>;
...
<?php $puesto_edit->ShowPageHeader(); ?>
<?php
$puesto_edit->ShowMessage();
?>
<form name="fpuestoedit" id="fpuestoedit" class="<?php echo $puesto_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($puesto_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $puesto_edit->Token ?>">
<?php } ?>
...
<script type="text/javascript">
ew_CreateCalendar("fpuestoedit", "x_fecha_inicio", "%d/%m/%Y");
</script>
<?php } ?>
</span>
...
<script type="text/javascript">
ew_CreateCalendar("fpuestoedit", "x_fecha_fin", "%d/%m/%Y");
</script>
<?php } ?>
</span>
...
<?php } ?>
<?php if ($puesto->estado->Visible) { // estado ?>
	<div id="r_estado" class="form-group">
		<label id="elh_puesto_estado" for="x_estado" class="col-sm-2 control-label ewLabel"><?php echo $puesto->estado->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $puesto->estado->CellAttributes() ?>>
<span id="el_puesto_estado">
<select data-table="puesto" data-field="x_estado" data-value-separator="<?php echo ew_HtmlEncode(is_array($puesto->estado->DisplayValueSeparator) ? json_encode($puesto->estado->DisplayValueSeparator) : $puesto->estado->DisplayValueSeparator) ?>" id="x_estado" name="x_estado"<?php echo $puesto->estado->EditAttributes() ?>>
<?php
if (is_array($puesto->estado->EditValue)) {
	$arwrk = $puesto->estado->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = ew_SameStr($puesto->estado->CurrentValue, $arwrk[$rowcntwrk][0]) ? " selected" : "";
		if ($selwrk <> "") $emptywrk = FALSE;		
?>
<option value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?>>
<?php echo $puesto->estado->DisplayValue($arwrk[$rowcntwrk]) ?>
</option>
<?php
	}
	if ($emptywrk && strval($puesto->estado->CurrentValue) <> "") {
?>
<option value="<?php echo ew_HtmlEncode($puesto->estado->CurrentValue) ?>" selected><?php echo $puesto->estado->CurrentValue ?></option>
<?php
    }
}
?>
...
<?php echo $puesto->estado->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
<input type="hidden" data-table="puesto" data-field="x_idpuesto" name="x_idpuesto" id="x_idpuesto" value="<?php echo ew_HtmlEncode($puesto->idpuesto->CurrentValue) ?>">
<div class="form-group">
	<div class="col-sm-offset-2 col-sm-10">
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $puesto_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
