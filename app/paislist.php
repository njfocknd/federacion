...
class cpais_list extends cpais {
	...
	function GetFilterList() {
		...
		$sFilterList = ew_Concat($sFilterList, $this->gentilicio_masculino->AdvancedSearch->ToJSON(), ","); // Field gentilicio_masculino
		$sFilterList = ew_Concat($sFilterList, $this->gentilicio_femenino->AdvancedSearch->ToJSON(), ","); // Field gentilicio_femenino
	...
	function RestoreFilterList() {
		...
		// Field gentilicio_masculino
		$this->gentilicio_masculino->AdvancedSearch->SearchValue = @$filter["x_gentilicio_masculino"];
		$this->gentilicio_masculino->AdvancedSearch->SearchOperator = @$filter["z_gentilicio_masculino"];
		$this->gentilicio_masculino->AdvancedSearch->SearchCondition = @$filter["v_gentilicio_masculino"];
		$this->gentilicio_masculino->AdvancedSearch->SearchValue2 = @$filter["y_gentilicio_masculino"];
		$this->gentilicio_masculino->AdvancedSearch->SearchOperator2 = @$filter["w_gentilicio_masculino"];
		$this->gentilicio_masculino->AdvancedSearch->Save();

		// Field gentilicio_femenino
		$this->gentilicio_femenino->AdvancedSearch->SearchValue = @$filter["x_gentilicio_femenino"];
		$this->gentilicio_femenino->AdvancedSearch->SearchOperator = @$filter["z_gentilicio_femenino"];
		$this->gentilicio_femenino->AdvancedSearch->SearchCondition = @$filter["v_gentilicio_femenino"];
		$this->gentilicio_femenino->AdvancedSearch->SearchValue2 = @$filter["y_gentilicio_femenino"];
		$this->gentilicio_femenino->AdvancedSearch->SearchOperator2 = @$filter["w_gentilicio_femenino"];
		$this->gentilicio_femenino->AdvancedSearch->Save();
	...
	function BasicSearchSQL($arKeywords, $type) {
		...
		$this->BuildBasicSearchSQL($sWhere, $this->gentilicio_masculino, $arKeywords, $type);
		$this->BuildBasicSearchSQL($sWhere, $this->gentilicio_femenino, $arKeywords, $type);
		return $sWhere;
	}
	...
	function SetUpSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			...
			$this->UpdateSort($this->gentilicio_masculino); // gentilicio_masculino
			$this->UpdateSort($this->gentilicio_femenino); // gentilicio_femenino
	...
	function ResetCmd() {
		...
		if (substr($this->Command,0,5) == "reset") {
			...
			if ($this->Command == "resetsort") {
				...
				$this->nombre->setSort("");
				$this->siglas->setSort("");
				$this->gentilicio_masculino->setSort("");
				$this->gentilicio_femenino->setSort("");
			}
	...
	function RenderListOptions() {
		...
		if ($Security->IsLoggedIn()) {
			...
			$body = "<a class=\"btn btn-default btn-sm ewRowLink ewDetail\" data-action=\"list\" href=\"" . ew_HtmlEncode("departamentolist.php?" . EW_TABLE_SHOW_MASTER . "=pais&fk_idpais=" . urlencode(strval($this->idpais->CurrentValue)) . "") . "\">" . $body . "</a>";
			$links = "";
			if ($GLOBALS["departamento_grid"]->DetailView && $Security->IsLoggedIn() && $Security->IsLoggedIn()) {
			...
			if ($GLOBALS["departamento_grid"]->DetailEdit && $Security->IsLoggedIn() && $Security->IsLoggedIn()) {
	...
	function SetupOtherOptions() {
		...
		$url = $this->GetAddUrl(EW_TABLE_SHOW_DETAIL . "=departamento");
		$caption = $Language->Phrase("Add") . "&nbsp;" . $this->TableCaption() . "/" . $GLOBALS["departamento"]->TableCaption();
		$item->Body = "<a class=\"ewDetailAddGroup ewDetailAdd\" title=\"" . ew_HtmlTitle($caption) . "\" data-caption=\"" . ew_HtmlTitle($caption) . "\" href=\"" . ew_HtmlEncode($url) . "\">" . $caption . "</a>";
		$item->Visible = ($GLOBALS["departamento"]->DetailAdd && $Security->IsLoggedIn() && $Security->IsLoggedIn());
		if ($item->Visible) {
	...
	function RenderListOptionsExt() {
		...
		$links = "";
		$btngrps = "";
		$sSqlWrk = "`idpais`=" . ew_AdjustSql($this->idpais->CurrentValue, $this->DBID) . "";

		// Column "detail_departamento"
		if ($this->DetailPages->Items["departamento"]->Visible) {
			$link = "";
			$option = &$this->ListOptions->Items["detail_departamento"];
			$url = "departamentopreview.php?t=pais&f=" . ew_Encrypt($sSqlWrk);
			$btngrp = "<div data-table=\"departamento\" data-url=\"" . $url . "\" class=\"btn-group\">";
			if ($Security->IsLoggedIn()) {			
				$label = $Language->TablePhrase("departamento", "TblCaption");
				$link = "<li><a href=\"#\" data-toggle=\"tab\" data-table=\"departamento\" data-url=\"" . $url . "\">" . $label . "</a></li>";			
				$links .= $link;
				$detaillnk = ew_JsEncode3("departamentolist.php?" . EW_TABLE_SHOW_MASTER . "=pais&fk_idpais=" . urlencode(strval($this->idpais->CurrentValue)) . "");
				$btngrp .= "<button type=\"button\" class=\"btn btn-default btn-sm\" title=\"" . $Language->TablePhrase("departamento", "TblCaption") . "\" onclick=\"window.location='" . $detaillnk . "'\">" . $Language->Phrase("MasterDetailListLink") . "</button>";
			}
			if ($GLOBALS["departamento_grid"]->DetailView && $Security->IsLoggedIn() && $Security->IsLoggedIn())
				$btngrp .= "<button type=\"button\" class=\"btn btn-default btn-sm\" title=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailViewLink")) . "\" onclick=\"window.location='" . $this->GetViewUrl(EW_TABLE_SHOW_DETAIL . "=departamento") . "'\">" . $Language->Phrase("MasterDetailViewLink") . "</button>";
			if ($GLOBALS["departamento_grid"]->DetailEdit && $Security->IsLoggedIn() && $Security->IsLoggedIn())
				$btngrp .= "<button type=\"button\" class=\"btn btn-default btn-sm\" title=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailEditLink")) . "\" onclick=\"window.location='" . $this->GetEditUrl(EW_TABLE_SHOW_DETAIL . "=departamento") . "'\">" . $Language->Phrase("MasterDetailEditLink") . "</button>";
			$btngrp .= "</div>";
			if ($link <> "") {
				$btngrps .= $btngrp;
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_pais\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_pais',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fpaislist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
...
<?php if ($pais->siglas->Visible) { // siglas ?>
	<?php if ($pais->SortUrl($pais->siglas) == "") { ?>
		<th data-name="siglas"><div id="elh_pais_siglas" class="pais_siglas"><div class="ewTableHeaderCaption"><?php echo $pais->siglas->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="siglas"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pais->SortUrl($pais->siglas) ?>',1);"><div id="elh_pais_siglas" class="pais_siglas">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $pais->siglas->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($pais->siglas->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($pais->siglas->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($pais->gentilicio_masculino->Visible) { // gentilicio_masculino ?>
	<?php if ($pais->SortUrl($pais->gentilicio_masculino) == "") { ?>
		<th data-name="gentilicio_masculino"><div id="elh_pais_gentilicio_masculino" class="pais_gentilicio_masculino"><div class="ewTableHeaderCaption"><?php echo $pais->gentilicio_masculino->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="gentilicio_masculino"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pais->SortUrl($pais->gentilicio_masculino) ?>',1);"><div id="elh_pais_gentilicio_masculino" class="pais_gentilicio_masculino">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $pais->gentilicio_masculino->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($pais->gentilicio_masculino->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($pais->gentilicio_masculino->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($pais->gentilicio_femenino->Visible) { // gentilicio_femenino ?>
	<?php if ($pais->SortUrl($pais->gentilicio_femenino) == "") { ?>
		<th data-name="gentilicio_femenino"><div id="elh_pais_gentilicio_femenino" class="pais_gentilicio_femenino"><div class="ewTableHeaderCaption"><?php echo $pais->gentilicio_femenino->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="gentilicio_femenino"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pais->SortUrl($pais->gentilicio_femenino) ?>',1);"><div id="elh_pais_gentilicio_femenino" class="pais_gentilicio_femenino">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $pais->gentilicio_femenino->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($pais->gentilicio_femenino->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($pais->gentilicio_femenino->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php
...
<a id="<?php echo $pais_list->PageObjName . "_row_" . $pais_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($pais->siglas->Visible) { // siglas ?>
		<td data-name="siglas"<?php echo $pais->siglas->CellAttributes() ?>>
<span id="el<?php echo $pais_list->RowCnt ?>_pais_siglas" class="pais_siglas">
...
<?php echo $pais->siglas->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($pais->gentilicio_masculino->Visible) { // gentilicio_masculino ?>
		<td data-name="gentilicio_masculino"<?php echo $pais->gentilicio_masculino->CellAttributes() ?>>
<span id="el<?php echo $pais_list->RowCnt ?>_pais_gentilicio_masculino" class="pais_gentilicio_masculino">
...
<?php echo $pais->gentilicio_masculino->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($pais->gentilicio_femenino->Visible) { // gentilicio_femenino ?>
		<td data-name="gentilicio_femenino"<?php echo $pais->gentilicio_femenino->CellAttributes() ?>>
<span id="el<?php echo $pais_list->RowCnt ?>_pais_gentilicio_femenino" class="pais_gentilicio_femenino">
